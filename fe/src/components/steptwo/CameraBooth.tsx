...
export default function CameraBooth({
  ...
  const handleActionClick = () => {
    ...
    } else if (!isAutoShooting && imageCount < 6) {
      startAutoShot();
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {flash && (
          <div className="absolute inset-0 bg-white opacity-90 animate-flash z-20" />
        )}
      </div>
      ...
      >
        ...
        >
          <ArrowLeftRight
            className="text-white cursor-pointer w-7 h-7"
            onClick={() => {
              const next = !isMirrorRef.current;
              isMirrorRef.current = next;
              setIsMirror(next);
            }}
          />
          {isPhotoTaken && (
